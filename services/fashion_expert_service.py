    ...
    async def _generate_response_from_reference_data(self, user_input: str, expert_type: FashionExpertType) -> str:
        ...
        try:
            ...
            if actual_combos:
                ...
                # 정직하고 신뢰할 수 있는 추천 (가짜 분석 제거)
                if expert_type == FashionExpertType.STYLE_ANALYST:
                    response_parts.append(f"키 175cm 기준으로 일반적으로 이런 스타일이 비율 좋아 보여요.")
                elif expert_type == FashionExpertType.TREND_EXPERT:
                    response_parts.append(f"요즘 트렌드를 보면 이런 스타일이 인기 있어요.")
                elif expert_type == FashionExpertType.COLOR_EXPERT:
                    response_parts.append(f"색상 조합으로 보면 베이지 톤이 잘 어울릴 것 같아요.")
                elif expert_type == FashionExpertType.FITTING_COORDINATOR:
                    response_parts.append(f"전체적으로 균형감이 좋은 조합이에요.")
                else:
                    response_parts.append(f"{template['prefix']} {combo['combination']}이 {template['focus']}에 잘 어울릴 것 같아요.")
                ...
                response_parts.append(f"구체적으로는 {items_str} 조합을 추천해요.")
                
                # 추가 정보 제공 (JSON 데이터 활용)
                if combo.get('occasion'):
                    response_parts.append(f"이 조합은 {combo['occasion']}에 특히 어울려요.")
