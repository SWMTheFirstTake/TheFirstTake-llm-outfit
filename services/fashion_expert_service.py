...
**필수 답변 형식:**
- 첫 문장: "색상+상의아이템에 색상+하의아이템이 잘 어울려" 또는 "색상+상의아이템과 색상+하의아이템 조합은 ~" 형식으로 시작
- 상의와 하의의 아이템명을 반드시 표기하고, JSON 데이터의 `top.item`, `bottom.item` 값을 그대로 사용
- 조사 사용: "~에" 사용 ("~랑", "~과", "~하고" 금지)
- 첫 문장 안에 색상 이론 근거를 바로 언급 (톤온톤, 명도 대비 등)
- 활용 가능한 색상 이론: 톤온톤, 명도 대비, 보색 관계, 색상 온도, 퍼스널컬러 등
- JSON 데이터의 실제 색상/아이템 정보를 사용하고 색상만 단독으로 언급하지 않기

**금지 사항:**
- 주관적 감탄으로 시작 금지 ("이 조합 좋아", "그 옷 조합 괜찮아", "야", "완전 좋아")
- 추상적 표현 금지 ("~한 느낌", "~한 분위기", "일반적으로")
- 고정 접두사 금지
- 상의/하의 아이템명을 생략하고 색상만 말하는 것

예시: "화이트 셔츠에 베이지 슬랙스가 잘 어울려. 톤온톤 원리라서 자연스럽고 명도 대비가 적당해서 세련돼 보여." """
            },
...
위의 구체적인 옷 조합 정보를 바탕으로 간결하고 핵심적인 패션 조언을 제공해주세요. 반드시 응답 시작 부분에 추천하는 옷 조합을 명확하게 명시하고, 반말로 간결하게 응답해주세요."""
    ...
    def _filter_unwanted_items(self, response: str) -> str:
        ...
        for old_term, new_term in shorts_replacements.items():
            if old_term in response:
                response = response.replace(old_term, new_term)
                print(f"🔄 쇼츠 변경: '{old_term}' → '{new_term}'")
        
        # 4. 정장 관련 표현을 포멀한 표현으로 변경
        formal_replacements = {
            '정장스러운': '포멀한',
            '정장적인': '포멀한', 
            '정장느낌': '포멀한 느낌',
