...
for combo in data['outfit_combinations']:
    if '데이트룩' in combo['combination']:
        daterook_combos.append(combo)

print(f'"데이트룩" 조합 {len(daterook_combos)}개 발견:')
print('=' * 50)

for i, combo in enumerate(daterook_combos, 1):
    ...
    print(f'   스트라이프 포함: {"예" if has_stripe else "아니오"}')
    print()

# 소개팅 키워드로 매칭되는 모든 조합 확인
print('=' * 50)
print('소개팅 키워드로 매칭되는 조합들:')
print('=' * 50)
...
for combo in data['outfit_combinations']:
    ...
    if '소개팅' in occasion or '데이트' in occasion:
        sogeting_matches.append(combo)

# 매칭 순서대로 정렬 (JSON 파일 순서대로)
for i, combo in enumerate(sogeting_matches, 1):
    items_str = ', '.join(combo['items']) if isinstance(combo['items'], list) else str(combo['items'])
    has_stripe = any('스트라이프' in item for item in (combo['items'] if isinstance(combo['items'], list) else [str(combo['items'])]))
    print(f'{i}. {combo["combination"]}')
    print(f'   아이템: {items_str}')
